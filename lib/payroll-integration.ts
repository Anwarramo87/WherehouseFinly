...
export class PayrollIntegration {
    ...
    try {
      ...
      const resignedPayrollData: ResignedEmployeePayrollData[] = resignedEmployees
        .map((employee) => {
          ...
          const payrollItem = payrollReport.items.find((item) => item.employeeId === employee.id);
  ...
  // NOTE: removeFromPayroll was deleted — it called POST /payroll/remove-employee,
  // which does not exist on the backend. The supported flow is the financial
  // settlement endpoint (POST /employees/financial-settlement).
