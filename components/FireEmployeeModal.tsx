...
const extractPaginationPages = (payload: unknown): number => {
  ...
  return Math.max(1, toNum(objectPayload.pagination?.pages ?? objectPayload.pages ?? 1));
};
    ...
    const fetchSettlement = async () => {
      ...
      try {
        ...
        const [
          ...
          apiClient.post(
            "/attendance/calculate-deductions",
            {
              periodStart,
              periodEnd,
        ...
        const provPayload =
          provisionalRes.status === "fulfilled" ? provisionalRes.value.data || {} : {};
        ...
        const inputsRaw =
          payrollInputsRes.status === "fulfilled"
            ? Array.isArray(payrollInputsRes.value.data)
              ? payrollInputsRes.value.data
              : payrollInputsRes.value.data?.data || []
            : [];
        ...
        const deductionsRaw =
          deductionsRes.status === "fulfilled"
            ...
            : [];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const autoInput = Array.isArray(deductionsRaw)
          ? deductionsRaw.find((d: any) => d.employeeId === employee.employeeId)
          : null;

        // salary record
        const salariesRaw =
          salariesRes.status === "fulfilled"
            ? Array.isArray(salariesRes.value.data)
              ? salariesRes.value.data
              : salariesRes.value.data?.data || []
            : [];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (grossSalary <= 0) {
          grossSalary =
            toNum((employee as Employee & { baseSalary?: unknown }).baseSalary) ||
        ...
        const lateMinutes =
          hasManual && (manualInput.lateMinutes ?? 0) > 0
            ? Number(manualInput.lateMinutes)
            : Math.max(Number(autoInput?.delayMinutes ?? 0), attendanceSummary.lateMinutes);
        const earlyLeaveMinutes =
          manualInput?.earlyLeaveMinutes ??
          Math.max(Number(autoInput?.earlyLeaveMinutes ?? 0), attendanceSummary.earlyLeaveMinutes);
        const overtimeMinutes =
          hasManual && (manualInput.overtimeRegularMinutes ?? 0) > 0
            ? Number(manualInput.overtimeRegularMinutes)
            : Math.max(Number(autoInput?.overtimeMinutes ?? 0), attendanceSummary.overtimeMinutes);
        const overtimeWeekendDays =
          hasManual && (manualInput.overtimeWeekendDays ?? 0) > 0
