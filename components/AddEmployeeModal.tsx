...
import { useState, useEffect } from "react";
import { createPortal } from "react-dom";
import { X, Loader2, Save, UserCog, Phone, User, Briefcase, ChevronRight, ChevronLeft, CalendarDays, Coins, CalendarHeart, Users, ShieldAlert } from "lucide-react";
import type { Employee } from "@/types/employee";
...
// دالة لمعالجة الأرقام القادمة من الباك إند
const asText = (value: unknown) => {
  if (value === null || value === undefined) return "";
  if (typeof value === "object" && value && "$numberDecimal" in value) {
    const decimal = (value as { $numberDecimal?: string }).$numberDecimal;
...
// دالة لتنسيق الرقم وإضافة فواصل الآلاف (مثال: 500000 -> 500,000)
const formatNumberWithCommas = (val: string) => {
  if (!val) return "";
  const numericValue = val.replace(/\D/g, ""); // مسح أي شيء ليس رقماً
  return numericValue.replace(/\B(?=(\d{3})+(?!\d))/g, ","); // إضافة الفاصلة
};
...
const buildInitialFormData = (initialData: EmployeeWithExtendedFields | null | undefined, nextSuggestedId: string) => {
  if (initialData) {
    const rawSalary = asText(initialData.monthlySalary || initialData.hourlyRate);
    const formattedSalary = formatNumberWithCommas(rawSalary);
...
function AddEmployeeModalContent({ isOpen, onClose, onSave, isPending, initialData, nextSuggestedId = "EMP00001" }: Props) {
  const [step, setStep] = useState<1 | 2>(1);
  const [mobileError, setMobileError] = useState("");
  const [formData, setFormData] = useState(() => buildInitialFormData(initialData, nextSuggestedId));
  ...
  // دالة مخصصة للتعامل مع تغير الراتب
  const handleSalaryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const formatted = formatNumberWithCommas(e.target.value);
    setFormData({ ...formData, monthlySalary: formatted });
  };
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 p-6 relative">
          <form id="employeeForm" onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5 text-right relative z-10">
            ...
            <div className={`col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5 transition-all duration-500 ${step === 1 ? 'block animate-in slide-in-from-right-8' : 'hidden'}`}>
              ...
              <div>
                <label className="block text-sm font-bold text-slate-500 mb-2">كود الموظف (ID) - تلقائي محمي</label>
                <input
                  type="text" 
                  disabled 
                  className="w-full p-3.5 bg-[#263544]/20 border border-[#263544]/40 rounded-xl outline-none text-left font-mono font-bold text-slate-400 cursor-not-allowed shadow-sm select-none"
                  dir="ltr"
                  value={formData.employeeId}
                />
              </div>
            ...
            <div className={`col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5 transition-all duration-500 ${step === 2 ? 'block animate-in slide-in-from-left-8' : 'hidden'}`}>
              ...
              <div>
                <label className="block text-sm font-bold text-[#C89355] mb-2">الدور الوظيفي (الصلاحية)</label>
                <div className="relative group">
                  <select
                    required={step === 2}
                    className="w-full p-3.5 bg-[#1a2530] border border-[#263544] rounded-xl focus:ring-2 focus:ring-[#C89355]/30 focus:border-[#C89355] outline-none transition-all text-white font-bold shadow-inner cursor-pointer pr-11 appearance-none"
                  ...
                  <ShieldAlert className="absolute right-4 top-3.5 text-slate-500 group-focus-within:text-[#C89355] transition-colors pointer-events-none" size={20} />
                </div>
              </div>
              ...
              {/* الراتب الشهري يشغل السطر كاملاً وتم تغيير نوع الحقل إلى text لتمرير الفاصلة */}
              <div className="md:col-span-2">
                <label className="block text-sm font-bold text-[#C89355] mb-2">الراتب الشهري الأساسي (ل.س)</label>
                <div className="relative group">
                  <input
                    type="text" inputMode="numeric" required={step === 2} placeholder="مثال: 500,000"
                    className="w-full p-3.5 bg-[#101720] border border-[#263544] rounded-xl focus:ring-2 focus:ring-[#C89355]/30 focus:border-[#C89355] outline-none transition-all text-[#C89355] font-mono text-lg font-black shadow-inner pr-11 placeholder:text-slate-600"
                    value={formData.monthlySalary}
...
export default function AddEmployeeModal(props: Props) {
  const isMounted = typeof document !== "undefined";
  ...
  const modalKey = props.initialData?.employeeId ?? props.nextSuggestedId ?? "new";

  return <AddEmployeeModalContent key={modalKey} {...props} />;
}