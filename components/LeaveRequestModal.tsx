...
// إغلاق القائمة عند النقر خارجها
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const serverMsg = error && typeof error === "object" && "response" in error
        ? (error as { response?: { data?: { message?: string | string[] } } }).response?.data?.message || "تعذر إرسال الطلب حالياً. حاول مرة أخرى."
