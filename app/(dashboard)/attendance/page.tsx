...
export default function AttendancePage() {
  ...
  const [liveAttendanceEvent, setLiveAttendanceEvent] =
    useState<AttendanceRealtimeEventPayload | null>(null);
  const [isLeaveModalOpen, setIsLeaveModalOpen] = useState(false);
  const [selectedLeave, setSelectedLeave] = useState<LeaveRecord | null>(null);

  const [punchesModal, setPunchesModal] = useState<{
    employeeId: string;
    employeeName: string;
  } | null>(null);
  ...
  if (employeesLoading || leavesLoading || dailyViewLoading)
    return (
      <div className="relative min-h-[85vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 bg-white/40 p-8 rounded-3xl backdrop-blur-2xl border border-white/60 shadow-[0_20px_40px_rgba(38,53,68,0.1)]">
          <div className="w-14 h-14 border-4 border-[#C89355]/30 border-t-[#263544] rounded-full animate-spin" />
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
                ...
                {liveAttendanceEvent && (
                  <div className="mt-5 relative overflow-hidden rounded-2xl border border-white/80 bg-white/60 backdrop-blur-xl p-4 flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-start gap-3 relative z-10">
                      ...
                      <div>
                        <p className="text-sm font-black text-[#263544]">
                          {liveAttendanceEvent.message}
                        </p>
                        <p className="text-xs text-slate-500 mt-1 font-bold">
                          {liveAttendanceEvent.employeeName}{" "}
                          <span className="text-[#C89355] mx-1">|</span>
              ...
              <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto">
                ...
                <div className="relative overflow-hidden flex flex-wrap items-center gap-3 bg-white/60 backdrop-blur-xl p-4 rounded-2xl border border-white/80 shadow-sm group">
                  <div className="absolute inset-1 rounded-xl border border-dashed border-[#C89355]/30 pointer-events-none" />
                  <span className="relative z-10 bg-[#1a2530] text-[#C89355] px-4 py-1.5 rounded-xl text-xs font-black border border-[#C89355]/30">
                    حاضر: {stats.present}
                  </span>
                  <span className="relative z-10 bg-orange-50 text-orange-600 px-4 py-1.5 rounded-xl text-xs font-black border border-orange-100">
                    متأخر: {stats.late}
                  </span>
                  <span className="relative z-10 bg-red-50 text-red-600 px-4 py-1.5 rounded-xl text-xs font-black border border-red-100">
                    غائب: {stats.absent}
                  </span>
                  ...
                  <span className="relative z-10 text-[#263544] px-3 py-1 text-xs font-black">
                    {dailyViewFetching ? (
                      <span className="flex items-center gap-2">
                        <Loader2 size={12} className="animate-spin text-[#C89355]" /> تحديث...
                      </span>
                    ) : (
              ...
              <table className="w-full text-right border-collapse min-w-225">
                <thead>
                  <tr className="bg-white/40 border-b border-white/80">
                    <th className="p-5 text-xs font-black text-[#263544] uppercase tracking-wider text-center">
                      الموظف
                    </th>
                    <th className="p-5 text-xs font-black text-[#263544] uppercase tracking-wider text-center">
                      التاريخ
                  ...
                  {rows.length === 0 ? (
                    ...
                    rows.map((row) => (
                      ...
                      >
                        ...
                        <td className="p-4 text-center">
                          <span
                            className={`px-4 py-1.5 rounded-xl text-[11px] font-black border ${statusUi[row.status].classes}`}
                          >
                        ...
                        <td className="p-4 text-center">
                          {row.checkIn || row.checkOut ? (
                            <span className="inline-flex items-center justify-center gap-2 text-xs font-black">
                              {row.source === "device" ? (
                                <>
                                  <Fingerprint size={16} className="text-[#263544]" />
                                  <span className="text-[#263544]">جهاز</span>
                                </>
                              ) : (
                                <>
                                  <PencilLine size={16} className="text-[#C89355]" />
        ...
        <div className="fixed inset-0 z-9999 flex items-center justify-center bg-[#101720]/70 backdrop-blur-sm p-4">
          ...
          >
            ...
            <div className="p-8 flex-1 order-2 md:order-1 relative z-10">
              ...
              <label className="block text-sm font-black text-[#263544]/80 mb-2">
                حدد الوقت بدقة
              </label>
              <input
                type="time"
            ...
            <div className="bg-[#263544]/5 p-8 md:w-2/5 border-b md:border-b-0 md:border-r border-[#C89355]/20 flex flex-col justify-center items-center text-center order-1 md:order-2 relative z-10">
              ...
              <p className="text-[10px] text-[#263544]/60 font-black uppercase tracking-wider mb-1">
                تاريخ السجل
              </p>
