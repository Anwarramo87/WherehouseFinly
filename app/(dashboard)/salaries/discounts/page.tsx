...
export default function DiscountsPage() {
  ...
  // Exclude rewards — they are on the separate rewards page
  ...
  const groupedDiscounts = useMemo(() => {
    const groups: Record<string, { employeeId: string; name: string; totalDeductions: number; records: typeof recordsWithNames }> = {};
        ...
        <header className="mb-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-[#263544]/10 pb-6 relative">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl sm:text-3xl font-black text-[#263544] tracking-tight drop-shadow-sm">الخصومات والسلف</h1>
            </div>
                                    ...
                                    {group.records.map((record) => (
                                      <tr key={record.id} className="hover:bg-white transition-colors">
                                        <td className="py-3 px-4">
                                          <span className={`px-3 py-1 rounded-lg text-[10px] font-black border ${record.backendModel === 'advance' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : record.kind === 'penalty' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-slate-100 text-slate-700 border-slate-200'
                                            }`}>
                                            {record.type}
                                          </span>
                                        </td>
