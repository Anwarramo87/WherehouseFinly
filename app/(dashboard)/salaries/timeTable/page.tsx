...
// import { useState, useMemo } from "react";
// import dynamic from "next/dynamic";
// import { Clock, ChevronLeft, Search, Edit2, CalendarPlus, AlertTriangle, Banknote, Loader2 } from "lucide-react";
// import { useQueryClient } from "@tanstack/react-query";
// import { toast } from "react-hot-toast";
// import apiClient from "@/lib/api-client";
// import { SALARY_CONSTANTS } from "@/lib/salary-constants";
...
// import type { Salary } from "@/types/salary";
// import { toNumber } from "@/lib/number-utils";
// import { MonthPeriodSelector } from "@/components/MonthPeriodSelector"; // <--- الإضافة هنا
...
//           <div className="flex flex-wrap items-center gap-3">
//             {/* ── هنا وضعنا الكومبوننت الجديد بدلاً من الإدخال الأصلي ── */}
//             <MonthPeriodSelector
//               value={selectedMonth}
//               onChange={setSelectedMonth}
...
//                           <Loader2 size={16} className="animate-spin text-[#C89355] mx-auto" />
//                         ) : (
//                           <span className="inline-flex items-center justify-center px-3 py-1 rounded-full text-sm font-bold bg-emerald-100 text-emerald-700 border border-emerald-200 font-mono">
//                             {record.actualWorkDays !== null ? `${record.actualWorkDays} يوم` : "—"}
//                           </span>
//                         )}
//                       </td>
...
//                       <td className="px-6 py-4 text-center">
//                         <span className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-sm font-bold font-mono ${record.totalAbsencesLeaves > 0 ? "bg-rose-100 text-rose-700 border border-rose-200" : "bg-slate-100 text-slate-500"}`}>
//                           {record.totalAbsencesLeaves} يوم
//                         </span>
//                       </td>
...
//                             ))}
//                             {record.paidLeaveDays > 0 && (
//                               <span className="text-[10px] text-emerald-600 font-bold font-mono">{record.paidLeaveDays} أيام مدفوعة</span>
//                             )}
//                           </div>
//                         ) : (
//                           <span className="text-slate-400 text-xs">—</span>
...
//                           <Loader2 size={16} className="animate-spin text-[#C89355] mx-auto" />
//                         ) : (
//                           <span className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-sm font-bold font-mono ${record.totalDelayMinutes > 0 ? "bg-orange-100 text-orange-700 border border-orange-200" : "bg-slate-100 text-slate-500"}`}>
//                             {record.totalDelayMinutes} دقيقة
//                           </span>
//                         )}
//                       </td>
...
//                         {record.totalEarlyLeaveMinutes > 0 ? (
//                           <span
//                             className="inline-flex items-center gap-1 bg-red-50 text-red-600 border border-red-100 text-xs font-bold font-mono px-3 py-1 rounded-full"
//                             title="تم استبعاد الإجازات الساعية المعتمدة تلقائياً"
//                           >
//                             <AlertTriangle size={13} />
//                             {record.totalEarlyLeaveMinutes} دقيقة
...
//                         <div className="flex flex-col items-center gap-1">
//                           {record.totalOvertimeMinutes > 0 && (
//                             <span className="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold font-mono bg-teal-100 text-teal-700 border border-teal-200">
//                               {record.totalOvertimeMinutes} د. عادي
//                             </span>
//                           )}
//                           {record.totalOvertimeDays > 0 && (
//                             <span className="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-bold font-mono bg-violet-100 text-violet-700 border border-violet-200">
//                               {record.totalOvertimeDays * HOURS_PER_DAY * 60} د. جمعة
//                             </span>
//                           )}
//                           {record.totalOvertimeDays === 0 && record.totalOvertimeMinutes === 0 && (
...
//                         ) : record.grossSalary > 0 && record.earnedSalary !== null ? (
//                           <div className="flex flex-col items-center gap-1">
//                             <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-black font-mono bg-[#C89355]/10 text-[#C89355] border border-[#C89355]/30">
//                               <Banknote size={14} />
//                               {Math.round(record.earnedSalary).toLocaleString()} ل.س
//                             </span>
//                             {record.earnedSalary < record.grossSalary && (
...
//                       value={addDayStartDate}
//                       onChange={(e) => setAddDayStartDate(e.target.value)}
//                       className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white text-sm font-bold font-mono text-[#263544] focus:border-[#C89355] focus:ring-1 focus:ring-[#C89355]/30 outline-none"
//                     />
//                   </div>
//                   <div>
//                     <label className="block text-xs font-black text-slate-500 mb-1">إلى تاريخ</label>
